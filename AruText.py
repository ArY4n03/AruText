...
class YourTale:
    ...
    def __init__(self,master):
        ...
        self.master.title("AruText")
        self.GameDir = ""
        self.toplevelExists = False
        self.selected_file = ""
        self.selected_fileIndex = 0
        self.GameSettings = {'start_scene':"",'title':""}
        ...
        #self.master.bind("<Control-s>",self.save_sceneFile)
        #self.master.bind("<Control-n>",lambda : eu.newSceneTopLevel(self)) 

        self.startsceneLabel = ctk.CTkLabel(self.SettingsTab, text="Start Scene :",font=(None,30)).place(relx = 0.1,rely=0.3)
        self.startsceneEntry = ctk.CTkEntry(self.SettingsTab,placeholder_text=self.GameSettings['start_scene'],font=(None,30),width=400)
        self.startsceneEntry.place(relx=0.4,rely=0.3)

        self.TitleLabel = ctk.CTkLabel(self.SettingsTab, text="Title",font=(None,30)).place(relx = 0.1,rely=0.5)
        self.TitleEntry = ctk.CTkEntry(self.SettingsTab,placeholder_text=self.GameSettings['title'],font=(None,30),width=400)
    ...
    def update_SettingsWidget(self):
        self.startsceneEntry.delete(0,tk.END)
        self.startsceneEntry.insert(0,self.GameSettings['start_scene'])
        self.TitleEntry.delete(0,tk.END)
        self.TitleEntry.insert(0,self.GameSettings['title'])
    ...
    def OpenNewFolder(self):
        ...
        if fileDir:
            ...
            ut.load_scene_files(self,self.GameDir)
            ut.loadSettings(self)
            self.update_SettingsWidget()
